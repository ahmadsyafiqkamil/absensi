...
def test_with_real_user():
    """Test DOCX generation with real user account staff.konsuler1"""
    print("\n=== Testing with Real User Account: staff.konsuler1 ===\n")
    ...
    try:
        ...
        request = OvertimeSummaryRequest.objects.create(
            ...
            user=user,
            request_period='2025-08',
            request_title=f"Rekap Lembur {dj_timezone.now().strftime('%B %Y')}",
            request_description="Test dengan akun staff.konsuler1",
            include_overtime_details=True,
            include_overtime_summary=True,
            include_approver_info=True,
            status='approved'
        ...
        overtime_requests = []
        for req_date, hours, description in overtime_dates:
            overtime_req = OvertimeRequest.objects.create(
                employee=employee,
                user=user,
...
=== Analyzing Generated Document ===")
            ...
            print("✓ Real user test completed successfully")

        else:
            print("✗ DOCX generation failed")

    except Exception as e:
        print(f"✗ Error testing with real user: {str(e)}")
        import traceback
        traceback.print_exc()
