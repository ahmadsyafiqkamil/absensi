...
export async function GET() {
  try {
    ...
    // Fetch positions from backend (V2 admin route)
    const response = await fetch(`${getBackendUrl()}/api/v2/employees/admin/positions/`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`
      }
    });
...
export async function POST(req: Request) {
  try {
    ...
    const resp = await fetch(`${backendBase}/api/v2/employees/admin/positions/`, {
      method: 'POST',
      headers: {
