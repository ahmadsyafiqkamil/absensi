...
export default function MonthlyExportApproval() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to submit monthly export request');
      }

      const result = await response.json();
      setSuccessMessage('Permintaan export bulanan berhasil diajukan!');
      
      // Reset form
      setFormData({ export_period: '' });
      
      // Refresh data
      await fetchExportRequests();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to submit monthly export request');
    } finally {
      setRequesting(false);
    }
  };
  ...
  const handleDownload = async (requestId: number) => {
    try {
      ...
      setError(null);
                        ...
                        const response = await authFetch(`/api/overtime-requests/monthly_exports/${requestId}/download/`);
      ...
      let filename = 'monthly-overtime-export.docx';
      ...
      setSuccessMessage('File export berhasil didownload!');
    } catch (err) {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          📊 Export Data Lembur Bulanan
        </CardTitle>
        <CardDescription>
          Ajukan approval untuk export data lembur bulanan dengan 2-level approval system
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold mb-4">📝 Ajukan Export Bulanan Baru</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="export_period">Periode Export *</Label>
                <Input
                  id="export_period"
                  name="export_period"
                  type="month"
                  value={formData.export_period}
                  onChange={(e) => setFormData({ export_period: e.target.value })}
                  required
                  min={new Date().getFullYear() - 1 + '-01'}
                  max={new Date().getFullYear() + 1 + '-12'}
                />
                <p className="text-sm text-gray-500">
                  Pilih bulan dan tahun untuk export data lembur
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <Button 
                ...
                disabled={requesting || !formData.export_period}
              >
                {requesting ? '⏳ Mengajukan...' : '📤 Ajukan Export Bulanan'}
              </Button>
            </div>
          </form>
        </div>
        ...
        <div>
          <h3 className="text-lg font-semibold mb-4">📋 Riwayat Permintaan Export</h3>
                  ...
                  <div key={request.id} className="border rounded-lg p-4 hover:bg-gray-50">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <div className="font-medium">{getStatusBadge(request.status)}</div>
                      </div>
                    ...
                    <div className="flex justify-end gap-2 mt-4 pt-3 border-t">
                      {request.status === 'approved' && request.exported_file_path && (
                        <Button
                          onClick={() => handleDownload(request.id)}
                          disabled={downloading === request.id}
                          className="bg-green-600 hover:bg-green-700"
                        >
                          {downloading === request.id ? '⏳ Downloading...' : '📥 Download Export'}
                        </Button>
                      )}
                      ...
                      {request.status === 'rejected' && (
                        <Button
                          onClick={() => {
                            setFormData({ export_period: request.export_period });
