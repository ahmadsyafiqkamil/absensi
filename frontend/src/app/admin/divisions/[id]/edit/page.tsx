...
export default function EditDivisionPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!resp.ok) throw new Error(data.detail || 'Failed to load division')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setTimeout(() => router.push('/admin/divisions'), 1000)
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unknown error')
    } finally {
      setSaving(false)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header title="Edit Division" subtitle={`ID: ${id || '-'}`} username="Admin" role="admin" />
      <div className="max-w-xl mx-auto px-4 py-8">
        <Card>
          <CardHeader>
            <CardTitle>Division</CardTitle>
            <CardDescription>Update division name</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div>Loading...</div>
